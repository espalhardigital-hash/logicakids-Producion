...
import { getFase5Question, submitFase5Answer, getFase5Reading, closeFase5Rescate } from './Fase5Service';
  ...
  const rec = useMemo(() => {
    if (moduloId === 99) {
      return {
        ...
        mensaje: '¡Has dominado por completo todos los desafíos de la Fase 5! Tu agilidad de cálculo y razonamiento numérico son extraordinarios. ¡Prepárate para la Fase 3!',
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          ¡Has completado y dominado con éxito toda la **Fase 5: Desarrollo Numérico y Razonamiento**! Eres oficialmente un héroe matemático de LogicaKids Pro. 🛡️✨
        </motion.p>
...
const Fase5GameScreen: React.FC<Props> = ({ moduloId, nivelId, onComplete, onBack }) => {
  // 1-22: State Hooks
    ...
    <div className="f5-game-screen">
          ...
          <header className="f5-game-header-modern">
            ...
            <div className="f5-header-right-group">
              ...
              <div className="f5-header-badge-pill">
                ...
                <span className="f5-badge-level">FASE 2</span>
                <span className="f5-badge-divider">|</span>
              ...
              >
                ...
                {pregunta.tipo_pregunta === 'respuesta_numerica' && (
                  <div className="flex flex-col h-full justify-between">
                    <div className="f5-question-text-box"><div className={(pregunta.enunciado || '').length < 25 ? "f5-question-text short" : "f5-question-text"}>{pregunta.enunciado}</div></div>
                ...
                {pregunta.tipo_pregunta === 'multiple_opcion' && (
                  <div className="flex flex-col h-full justify-between">
                     <div className="f5-question-text-box"><div className="f5-question-text">{pregunta.enunciado}</div></div>
                ...
                {pregunta.tipo_pregunta === 'constructor_soluciones_chained' && (
                  <div className="flex flex-col h-full justify-between gap-4">
                    <div className="f5-question-text-box">
                      <div className="f5-question-text">{cleanEnunciado(pregunta.enunciado)}</div>
                    </div>
      ...
      <AnimatePresence>
        {showReading && readingData && (
          <Fase5TheoryModal readingData={readingData} moduleColor={moduleColor} onClose={() => setShowReading(false)} onAbort={() => isInitialReading ? onBack() : setShowReading(false)} isInitialReading={isInitialReading} userAvatar={userAvatar} />
        )}
        ...
        {showGraduation && (
          <Fase5PhaseGraduationModal studentName={studentName} onClose={() => { setShowGraduation(false); navigate('/map'); }} />
        )}
        {showCompletion && (
...
function MOCK_LECTURA(moduloId: number, nivelId: number): Fase5Lectura {
...
function MOCK_PREGUNTA(moduloId: number, nivelId: number): Fase5Pregunta {
