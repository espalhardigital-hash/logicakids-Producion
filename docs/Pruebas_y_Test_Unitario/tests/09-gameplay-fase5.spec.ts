...
test.describe('09 - Gameplay Fase 5 (Geometría Plana y Medidas)', () => {
  ...
  test.beforeAll(() => {
    try {
      execSync(
        `docker exec logicakids_local_db psql -U logicakids_local_user -d logicakids_local -c "UPDATE alumnos SET fase_actual_id = 5 WHERE user_id = (SELECT id FROM users WHERE email = '${process.env.TEST_EMAIL || 'prueba@gmail.com'}');"`
      );
      execSync(
