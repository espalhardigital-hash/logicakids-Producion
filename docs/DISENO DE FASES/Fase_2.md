Aquí tienes el documento completo y actualizado, listo para que lo copies y pegues directamente en tu sistema o documentación. He integrado los nuevos textos profesionales (Textos de Aprendizaje) manteniendo intacta la estructura original de diccionarios, ejemplos, interactivos y reglas del servidor.
...
Texto de Aprendizaje (Concepto Central): Una ecuación matemática funciona bajo el principio de equivalencia: es una balanza en perfecto equilibrio donde el signo igual (=) es el centro. Si desconocemos un valor inicial al que se le sumó o restó una cantidad, podemos deducirlo utilizando las operaciones inversas. La resta es la operación inversa de la suma, y viceversa. Si un valor se añadió a un lado de nuestra ecuación, para mantener el equilibrio y descubrir la incógnita, debemos aplicar la sustracción en el sentido contrario.
...
Paso 1: Deshacemos primero la operación de jerarquía menor (la suma). Pasamos el +3 restando: 2 × X = 11 - 3 → 2 × X = 8.
...
Texto de Aprendizaje (Concepto Central): Al consolidar múltiples valores financieros, aplicamos la adición con reagrupación decimal. El proceso es sistemático: sumamos las partes fraccionarias por un lado y las enteras por el otro. Si la suma de las fracciones alcanza o supera las 100 unidades (por ejemplo, al sumar 0,75 y 0,50), aplicamos una reagrupación base 10: convertimos esos 100 centavos en 1 unidad entera, la cual se transfiere inmediatamente al registro de los números enteros, dejando únicamente el residuo en la columna decimal.

El Diccionario del Nivel:
Suma de Precios:

* Suma las partes enteras (pesos).
* Suma los centavos.
* Si los centavos dan 100 o más, sumas 1,00 al entero y restas 100 a los centavos.
...
Texto de Aprendizaje (Concepto Central): En el modelado matemático, las operaciones suelen presentar una dependencia secuencial. Esto significa que el resultado obtenido en tu primera ecuación se convierte inmediatamente en el dato de entrada (input) para tu siguiente cálculo. Mantener la concentración en el traspaso de este valor es vital; un cálculo correcto en la primera etapa perderá todo su valor si no se encadena lógicamente con la premisa final del problema.
...
Paso 2 (Seguir reloj): Tenía 10 estampas y perdió 2 → le quedan 10 - 2 = 8 estampas.
Paso 3 (Multiplicador final): Su hermano le duplica lo que le quedaba → 8 × 2 = 16 estampas.
...
* **Inserción de Teoría (niveles_teoria_pool):**
Carga de forma pre-renderizada toda la información correspondiente a la Fase A de aprendizaje (Textos de Aprendizaje, Diccionario, Ejemplos e Interactivos con sus correspondientes JSONB de feedbacks).
* **Inserción de Preguntas de Práctica Libre (preguntas_pool):**
...
1. 1 Pregunta Original (con `estructura_padre_id` definido).
2. 3 Variantes Espejo (compartiendo el mismo `estructura_padre_id`).
...
* **Inserción del Pool de Desafíos (preguntas_pool):**
...
* **Server-Authoritative Estricto:** Toda la validación de respuestas, control de temporizadores en los desafíos, gestión de puntajes y reglas de avance (como el Early Exit) se procesan asíncronamente en el backend. El frontend (`Fase2GameScreen.tsx`) es puramente reactivo, eliminando cualquier rastro de lógicas locales offline.
* **Interactividad Modular:** Las respuestas numéricas (Módulos 1 al 3 y Desafío Final) se capturan mediante la sinergia de una Caja de Entrada Personalizada con input oculto y el Teclado Numérico Virtual, garantizando una experiencia fluida tanto en PC como en tablets.
* **Mecánica de Feedback Ágil y No Bloqueante:**
  * **Progresión por Completitud:** La aprobación del nivel en Práctica Libre se basa al 100% en completar las 15 familias de preguntas únicas (intentadas o bypasseadas). No se imponen bloqueos de precisión mínima para avanzar; el alumno aprueba el nivel al completar la batería de 15 familias, aunque falle todas, asegurando un aprendizaje libre de frustración y fricción. La precisión se registra de manera estadística para diagnóstico pedagógico.
  * **Cierre de Rescate Fluidificado (Bypass sin Anti-Spam):** El panel de explicación profunda del Bloque de Rescate se puede omitir libremente llamando al endpoint `/fase2/cerrar-rescate`. Este endpoint registra el bypass en la base de datos de manera server-authoritative (`BYPASS_EXPLICACION`), avanza la completitud del alumno y resetea a cero las fallas consecutivas del bucle espejo de forma fluida y sin requerir ninguna caja de texto ni transcripción anti-spam.
  * **Reinicio por Reload (F5):** Si el alumno sale o refresca el navegador durante la realización de un nivel de Práctica Libre incompleto, al volver a cargar con `reload=True` el backend elimina los intentos parciales de esa sección y restablece el progreso al `0%` para asegurar la continuidad y asimilación conceptual.
* **Normalización Monetaria en Centavos:** Para evitar inconsistencias de redondeo y facilitar las comparaciones numéricas, todos los valores y cálculos del Módulo 3 (Tienda Matemática) se manejan en centavos en la base de datos (p. ej., un chocolate de R$ 2,50 se normaliza y almacena como `250`). El frontend recibe y muestra R$ con coma decimal únicamente para fines visuales, mientras que el intercambio y procesamiento en los endpoints es en centavos enteros.