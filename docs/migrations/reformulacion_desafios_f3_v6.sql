-- ============================================================
-- REFORMULACIÓN PEDAGÓGICA — DESAFÍOS FASE 3 (v6-12-CONTEXTOS-CORREGIDA)
-- ============================================================
...
-- Extrae del backup b para evitar encadenar sobre preguntas ya modificadas
-- ============================================================
UPDATE preguntas p
SET enunciado = CASE (p.id % 12)
  WHEN 0 THEN
    'En una fiesta había ' ||
    COALESCE(substring(b.enunciado FROM 'tiene (\d+) globos'), '?') ||
    ' globos de colores. Al terminar, ' ||
    COALESCE(substring(b.enunciado FROM 'regal. (\d+) globos'), '?') ||
...
END
FROM _backup_desafios_f3 b
WHERE p.id = b.id AND p.fase_id = 3 AND p.seccion IN (1011, 1012, 1013);
...
SET enunciado = CASE (p.id % 12)
  WHEN 0 THEN
    ...
    COALESCE(substring(b.enunciado FROM 'con (\d+) cajas'), '?') ||
    ' pasajeros. En la primera parada bajaron ' ||
    COALESCE(substring(b.enunciado FROM 'Entrega (\d+) cajas'), '?') ||
    ' y subieron ' ||
    COALESCE(substring(b.enunciado FROM 'carga (\d+) cajas'), '?') ||
...
END
FROM _backup_desafios_f3 b
WHERE p.id = b.id AND p.fase_id = 3 AND p.seccion IN (2011, 2012, 2013);
...
SET enunciado = CASE (p.id % 12)
  WHEN 0 THEN
    ...
    COALESCE(substring(b.enunciado FROM 'cuestan R. (\d+),'), '?') ||
    ' en total. 3 chocolates con el mismo refresco cuestan $' ||
    COALESCE(substring(b.enunciado FROM 'cartuchera cuestan R. (\d+)'), '?') ||
...
END
FROM _backup_desafios_f3 b
WHERE p.id = b.id AND p.fase_id = 3 AND p.seccion IN (3011, 3012, 3013);
...
SET enunciado = CASE (p.id % 12)
  WHEN 0 THEN
    'La panadería horneó ' ||
    COALESCE(substring(b.enunciado FROM 'hizo (\d+) panes'), '?') ||
    ' panes y los puso en cajas de ' ||
    COALESCE(substring(b.enunciado FROM 'bolsas de (\d+) panes'), '?') ||
...
END
FROM _backup_desafios_f3 b
WHERE p.id = b.id AND p.fase_id = 3 AND p.seccion IN (4011, 4012, 4013);
...
SET enunciado = CASE (p.id % 12)
  WHEN 0 THEN
    ...
    COALESCE(substring(b.enunciado FROM 'tarda (\d+) minutos por vuelta'), '?') ||
    ' minutos y el otro en ' ||
    COALESCE(substring(b.enunciado FROM 'compa.ero tarda (\d+) minutos'), '?') ||
...
END
FROM _backup_desafios_f3 b
WHERE p.id = b.id AND p.fase_id = 3 AND p.seccion IN (5011, 5012, 5013);
...
END
FROM _backup_desafios_f3 b
WHERE p.id = b.id AND p.fase_id = 3 AND p.seccion = 99099;
